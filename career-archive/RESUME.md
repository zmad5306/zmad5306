ZACH MADDOX
Senior Software Engineer & Cloud Architect
📧 [dev@zachmaddox.dev](mailto:dev@zachmaddox.dev) | 🌐 [https://zachmaddox.dev](https://zachmaddox.dev) | 💼 LinkedIn: [https://www.linkedin.com/in/zachary-maddox-693a9862](https://www.linkedin.com/in/zachary-maddox-693a9862) | 🐙 GitHub: [https://github.com/zmad5306](https://github.com/zmad5306)

---

## PROFESSIONAL SUMMARY

Experienced **Software Engineer and Cloud Architect** with nearly **20 years** of expertise spanning government and private-sector modernization. Skilled in designing, implementing, and maintaining **secure, scalable, cloud-native applications** across **Azure** and **AWS**. Adept at transforming legacy systems into resilient, maintainable architectures using **Java (Spring Boot)**, **.NET**, and **containerized microservices**.

Champion of **AI-augmented engineering**, treating AI as a force multiplier — accelerating delivery without sacrificing architectural intent or human oversight.

---

## CORE COMPETENCIES

- **Languages:** Java, C#, Python, Kotlin, JavaScript, TypeScript, SQL
- **Frameworks:** Spring Boot, ASP.NET Core, Angular, React, MuleSoft ESB, Flask, Express
- **Cloud Platforms:** Azure (Functions, App Service, API Management, ARM, ADF), AWS (ECS, ECR, S3, CloudFormation)
- **Databases:** PostgreSQL, SQL Server, DB2, Oracle, TimescaleDB, MongoDB, Redis
- **DevOps & Tools:** Docker, Kubernetes, Terraform, Azure DevOps, Bamboo, GitHub Actions
- **Security:** OAuth2 / OIDC, bearer tokens, RBAC, CAIA integration, secure API gateway design
- **Data & Analytics:** Azure Data Factory, SSIS, ETL, schema design, SQL optimization
- **Architecture:** Microservices, event-driven systems, distributed transactions, observability, maintainability

---

## PROFESSIONAL EXPERIENCE

### **JBS.dev** — Senior Software Engineer / Cloud Architect

_2021 – Present_

- Designed and implemented **energy-sector sustainability platform** microservices in **Java (Spring Boot 17)** and **Python**, supporting emissions tracking, goal modeling, and IoT integration.
- Led **migration to AWS ECS**, introducing containerization and full **CI/CD automation**.
- Built **PostgreSQL-backed real-time reporting APIs**, optimizing query performance and schema design.
- Managed **Azure Data Factory pipelines** and SSIS automation for ETL orchestration.
- Drove **microservice security engineering**, implementing bearer tokens, RBAC, and consistent API policies.
- Advocated for AI-assisted workflows, integrating automated code validation and static analysis.

**Key Achievements:**
✔ Delivered reliable, high-performance energy analytics at scale.
✔ Improved deployment velocity 3× through CI/CD and containerization.
✔ Strengthened security posture and observability across all microservices.

---

### **U.S. Department of the Treasury – Bureau of the Fiscal Service**

**Architect / Team Lead (2016 – 2021)** | **Developer / Senior Developer (2007 – 2016)**

- **Led modernization** of national financial systems — from COBOL mainframe services to Azure-hosted APIs.
- Architected and delivered the **Savings Bond Valuation & Verification (SBVV)** and **Treasury Check Verification System (TCVS)** using **C#/.NET Azure Functions**, Angular, and Azure SQL.
- **Pioneered Azure adoption** within the Bureau — leading design, governance, and first FedRAMP High-compliant deployments.
- Developed **Spring Boot modernization** of the Savings Bond Image Viewer, integrating with **CICS** and **IBM MQ**.
- Implemented identity federation via **CAIA / OIDC**, enabling secure MFA across systems.
- Championed **MuleSoft ESB** enterprise integration, leading API design and reuse strategy.

**Key Achievements:**
✔ Delivered first production **Azure deployment** for the U.S. Treasury.
✔ Designed cross-system authentication and audit services for financial integrity.
✔ Mentored multiple development teams on cloud-first and containerized architectures.

---

## HIGH-IMPACT CONTRIBUTIONS

- **SBVV:** Designed and delivered Azure-hosted valuation API serving U.S. financial institutions.
- **TCVS:** Developed fraud-prevention verification API for Treasury checks, preventing high-value fraud.
- **Azure Platform Leadership:** Led migration of savings bond data (billions of records) from DB2 to Azure SQL; managed infrastructure under FedRAMP High controls.
- **AWS Migration:** Delivered full containerized migration of energy-sector platform with CI/CD automation and improved observability.

---

## PERSONAL PROJECTS & RESEARCH

### ⚡ **ScoreCard — Observability-Driven Distributed Transaction Engine**

**Tech Stack:** Java 21 • Spring Boot • REST API • OpenAPI • PostgreSQL • JMS/ActiveMQ • Docker • Maven
**Links:** GitHub — [https://github.com/zmad5306/ScoreCard](https://github.com/zmad5306/ScoreCard) | Research Blog — [https://zachmaddox.dev/blog/2025/12/14/score-card.html](https://zachmaddox.dev/blog/2025/12/14/score-card.html)

**ScoreCard** is an advanced R&D project investigating how to coordinate multi-step, cross-service workflows **without 2PC, global locks, or tightly coupled orchestrators**. It models business transactions as graphs of dependent actions and uses **observability signals** — action reports, correlation IDs, and service metadata — to reconstruct end-to-end execution.

It demonstrates how microservices can remain **autonomous** while still participating in coherent, traceable, and recoverable workflows.

**Highlights:**

- Explicit modeling of services, actions, and dependency graphs
- Real-time tracking of action states and metadata
- Failure detection with compensation flows (e.g., reversing partial bank transfers)
- Developer UIs for live monitoring and workflow design
- Example microservices demonstrating sequencing, retries, and resilience patterns

---

### 🕵️ **ActionWatch — GitHub Actions Monitoring & Insights**

A TypeScript / Next.js platform that visualizes GitHub Actions pipelines in real time with authentication, TOTP security, and WebSocket-based event streaming.

---

### 🌍 **Chunking Collector — Open Source Java Library**

**Tech Stack:** Java 8+ • Streams API • Maven Central • GitHub Actions • JavaDoc
**Links:** GitHub — [https://github.com/zmad5306/ChunkingCollector](https://github.com/zmad5306/ChunkingCollector) | Maven Central — [https://central.sonatype.com/artifact/dev.zachmaddox/chunking-collector/overview](https://central.sonatype.com/artifact/dev.zachmaddox/chunking-collector/overview) | JavaDoc — [https://zachmaddox.dev/chunking-collector/latest/](https://zachmaddox.dev/chunking-collector/latest/)

A zero-dependency Java library that extends the Stream API for **batching, segmentation, and windowing**. Implements fast, memory-safe chunking via native Collector implementations.

> _Open source is how I give back — creating tools that make elegant, reliable engineering accessible to everyone._

### 🏠 Homelab & Self-Hosting

Operates a self-hosted **Linux server** running containerized applications (Nextcloud, Pi-hole, WireGuard VPN) via Docker Compose — a sandbox for continuous learning, networking, and automation.

---

## CERTIFICATIONS

- **☕ Azul PartnerConnect:** Certified OpenJDK Migration Services & Advisory Partner, Platform Prime & Core Certified Engineer (2024)
- **☁️ Microsoft Azure:** Certified Solutions Architect Expert (2023), Administrator Associate (2023)
- **🌀 Scrum Alliance:** Certified ScrumMaster (CSM) (2019)

---

## EDUCATION

**M.S., Information Systems Management** — Keller Graduate School of Management, 2009
**B.S., Computer Information Systems** — DeVry University, 2007
**A.A.B., Computer Systems / Programming** — Washington State Community College, 2004

---

## PRINCIPLES OF AI-AUGMENTED ENGINEERING

1. **Humans Design, AI Builds** – Engineers define purpose; AI executes within boundaries.
2. **Guardrails Before Generation** – Context, safety, and clarity precede automation.
3. **Trust, But Verify** – AI drafts, engineers validate.
4. **Engineer the System, Not the Syntax** – Design for maintainability, not verbosity.
5. **Augment, Don’t Abdicate** – AI accelerates; judgment guides.

> _AI doesn’t replace expertise — it replaces tedium. In capable hands, it magnifies engineering excellence._
